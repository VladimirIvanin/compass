...
class SassExtensionsTest < Test::Unit::TestCase
  ...
  def test_math_functions
    ...
    assert_equal "0", evaluate("sin(pi())")
    assert_equal "1",   evaluate("sin(pi() / 2)")
    assert_equal "0",   evaluate("sin(180deg)")
    assert_equal "-1",  evaluate("sin(3* pi() / 2)")
    assert_equal "-1", evaluate("cos(pi())")
    assert_equal "1", evaluate("cos(360deg)")
    assert_equal "1.0471975512", evaluate("acos(0.5)")
    assert_equal "1.0471975512", evaluate("acos(100px/200px)")
    assert_equal "-0.1760459465", evaluate("sin(270)")
    assert_equal "1", evaluate("cos(2*pi())")
    assert_equal "0",   evaluate("cos(pi() / 2)")
    assert_equal "0",  evaluate("cos(3* pi() / 2)")
    assert_equal "0",  evaluate("tan(pi())")
    assert_equal "0.463647609",  evaluate("atan(0.5)")
    assert_equal "0", evaluate("tan(360deg)")
